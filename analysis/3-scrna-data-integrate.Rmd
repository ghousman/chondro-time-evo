...
#Read in files
data <- readRDS(paste0("./data/data",filter,".rds"))
...
while (i <= length(batch)) {
  ...
  if (batch[i]=="batch2") {cmo_sub <- cmo[c(1,3,4,5,6)]}
  
  while (j <= length(cmo_sub)) {
    
    print(cmo_sub[j])
  
    tmp <- subset(data[[i]], subset=ComboRef_0.9==cmo_sub[j])
...
#objects
saveRDS(objects[1:34], file=paste0("./data/data",filter,".log.total-ind-stg.rds"))

#merge all stages of differentiation from each individual (.log.total.ind)
objects[[35]] <- merge(objects[[1]], y=c(objects[[3]],objects[[5]],objects[[7]],objects[[9]],objects[[11]]),
                       add.cell.ids=c("H1-d00","H1-d0","H1-d7-m","H1-d14-m","H1-d7-c","H1-d14-c"))
objects[[36]] <- merge(objects[[2]], y=c(objects[[4]],objects[[6]],objects[[8]],objects[[10]],objects[[12]]),
                       add.cell.ids=c("C1-d00","C1-d0","C1-d7-m","C1-d14-m","C1-d7-c","C1-d14-c"))
objects[[37]] <- merge(objects[[13]], y=c(objects[[15]],objects[[17]],objects[[19]],objects[[21]],objects[[23]]),
                       add.cell.ids=c("H2-d00","H2-d0","H2-d7-m","H2-d14-m","H2-d7-c","H2-d14-c"))
objects[[38]] <- merge(objects[[14]], y=c(objects[[16]],objects[[18]],objects[[20]],objects[[22]],objects[[24]]),
                       add.cell.ids=c("C4-d00","C4-d0","C4-d7-m","C4-d14-m","C4-d7-c","C4-d14-c"))
objects[[39]] <- merge(objects[[25]], y=c(objects[[27]]),
                       add.cell.ids=c("H5-d0","H5-d14-c")) #no "H5-d7-c"
objects[[40]] <- merge(objects[[26]], y=c(objects[[28]]),
                       add.cell.ids=c("C5-d0","C5-d14-c")) #no "C5-d7-c"
objects[[41]] <- merge(objects[[29]], y=c(objects[[31]],objects[[33]]),
                       add.cell.ids=c("H3-d0","H3-d7-c","H3-d14-c"))
objects[[42]] <- merge(objects[[30]], y=c(objects[[32]],objects[[34]]),
                       add.cell.ids=c("C2-d0","C2-d7-c","C2-d14-c"))
saveRDS(objects[35:42], file=paste0("./data/data",filter,".log.total-ind.rds"))

#merge all chondrogenic stages of differentiation from each individual (.log.chond.ind)
objects[[43]] <- merge(objects[[1]], y=c(objects[[3]],objects[[9]],objects[[11]]),
                       add.cell.ids=c("H1-d00","H1-d0","H1-d7-c","H1-d14-c"))
objects[[44]] <- merge(objects[[2]], y=c(objects[[4]],objects[[10]],objects[[12]]),
                       add.cell.ids=c("C1-d00","C1-d0","C1-d7-c","C1-d14-c"))
objects[[45]] <- merge(objects[[13]], y=c(objects[[15]],objects[[21]],objects[[23]]),
                       add.cell.ids=c("H2-d00","H2-d0","H2-d7-c","H2-d14-c"))
objects[[46]] <- merge(objects[[14]], y=c(objects[[16]],objects[[22]],objects[[24]]),
                       add.cell.ids=c("C4-d00","C4-d0","C4-d7-c","C4-d14-c"))
objects[[47]] <- merge(objects[[25]], y=c(objects[[27]]),
                       add.cell.ids=c("H5-d0","H5-d14-c")) #no "H5-d7-c"
objects[[48]] <- merge(objects[[26]], y=c(objects[[28]]),
                       add.cell.ids=c("C5-d0","C5-d14-c")) #no "C5-d7-c"
objects[[49]] <- merge(objects[[29]], y=c(objects[[31]],objects[[33]]),
                       add.cell.ids=c("H3-d0","H3-d7-c","H3-d14-c"))
objects[[50]] <- merge(objects[[30]], y=c(objects[[32]],objects[[34]]),
                       add.cell.ids=c("C2-d0","C2-d7-c","C2-d14-c"))
saveRDS(objects[43:50], file=paste0("./data/data",filter,".log.chond-ind.rds"))

#merge all mesensphere stages of differentiation from each individual (.log.mesen.ind)
objects[[51]] <- merge(objects[[1]], y=c(objects[[3]],objects[[5]],objects[[7]]),
                       add.cell.ids=c("H1-d00","H1-d0","H1-d7-m","H1-d14-m"))
objects[[52]] <- merge(objects[[2]], y=c(objects[[4]],objects[[6]],objects[[8]]),
                       add.cell.ids=c("C1-d00","C1-d0","C1-d7-m","C1-d14-m"))
objects[[53]] <- merge(objects[[13]], y=c(objects[[15]],objects[[17]],objects[[19]]),
                       add.cell.ids=c("H2-d00","H2-d0","H2-d7-m","H2-d14-m"))
objects[[54]] <- merge(objects[[14]], y=c(objects[[16]],objects[[18]],objects[[20]]),
                       add.cell.ids=c("C4-d00","C4-d0","C4-d7-m","C4-d14-m"))
saveRDS(objects[51:54], file=paste0("./data/data",filter,".log.mesen-ind.rds"))

#merge all collections (.log.tot)
objects[[55]] <- merge(objects[[1]],
                        y=c(objects[[3]],objects[[5]],objects[[7]],objects[[9]],objects[[11]],
                            ...
                            objects[[25]],objects[[27]],
                            objects[[26]],objects[[28]],
                            objects[[29]],objects[[31]],objects[[33]],
                            objects[[30]],objects[[32]],objects[[34]]),
                        add.cell.ids=c("H1-d00","H1-d0","H1-d7-m","H1-d14-m","H1-d7-c","H1-d14-c",
...
saveRDS(objects[55], file=paste0("./data/data",filter,".log.total.rds"))
...
#skip objects[[31]], objects[[32]], objects[[33]], objects[[34]] - too few samples
...
#Save data
saveRDS(objects[1:34], file=paste0("./data/data",filter,".log.ind-stg.rds"))
saveRDS(objects[35:42], file=paste0("./data/data",filter,".log.total-ind.rds"))
saveRDS(objects[43:50], file=paste0("./data/data",filter,".log.chond-ind.rds"))
saveRDS(objects[51:54], file=paste0("./data/data",filter,".log.mesen-ind.rds"))
saveRDS(objects[55], file=paste0("./data/data",filter,".log.total.rds"))
...
obj.set1 <- readRDS(paste0("./data/data",filter,".log.total-ind.rds"))
obj.set2 <- readRDS(paste0("./data/data",filter,".log.chond-ind.rds"))
obj.set3 <- readRDS(paste0("./data/data",filter,".log.mesen-ind.rds"))
...
pdf(file=paste0("./output/data-qc-post",str_remove(filter,"."),"-subsets-batchviz.pdf"), onefile=TRUE, width=36, height=81)
i=1
for (i in 1:length(objects)) {
  text <- paste0("subset ",i)
  data <- objects[[i]]
...
```{bash, eval=FALSE}
/project2/gilad/ghousman/chondro-human-chimp/hc-chondro-time/chondro-time-evo
...
..
..
ADD TOTAL AND MESENSPHERE DATA INTEGRATION AND QC
..
..
..
...
obj <- readRDS(paste0("./data/data",filter,".log.chond-ind.rds"))
...
#save data
saveRDS(integrate, file=paste0("./data/integrated/data",filter,".log.chond-ind.int.",genes,".",dimred,".",kweigh,".",regout,".rds"))
...
#Load information of integrated data (chondrogenic cells) to be examined
...
obj.info <- list(c("int.Var.cca.50.non",paste0("./data/integrated/data",filter,".log.chond-ind.int.Var.cca.50.non.rds")),
                 c("int.Var.cca.50.reg",paste0("./data/integrated/data",filter,".log.chond-ind.int.Var.cca.50.reg.rds")),
                 c("int.No0.cca.50.non",paste0("./data/integrated/data",filter,".log.chond-ind.int.No0.cca.50.non.rds")),
                 c("int.No0.cca.50.reg",paste0("./data/integrated/data",filter,".log.chond-ind.int.No0.cca.50.reg.rds")),
                 c("int.19k.cca.50.non",paste0("./data/integrated/data",filter,".log.chond-ind.int.19k.cca.50.non.rds")))
#c("int.19k.cca.50.reg",paste0("./data/integrated/data",filter,".log.chond-ind.int.19k.cca.50.reg.rds"))

batch <- c("Stage","Species","Collection","Pair","Individual","Sample","Sex","Age","nCount_RNA","nFeature_RNA","percent.mt","S.Score","G2M.Score","Phase")

#Calculate and plot correlation of batch with first 10 PCs and UMAP
pdf(file=paste0("./output/data-qc-post",str_remove(filter,"."),"-subsets-chondro-ind-batchcor.pdf"), onefile=TRUE, width=7, height=7)
i=1
...
#Visualize batch effects in PC and UMAP space
pdf(file=paste0("./output/data-qc-post",str_remove(filter,"."),"-subsets-chondro-ind-batchviz.pdf"), onefile=TRUE, width=42, height=48)
i=1
for (i in 1:length(obj.info)) {
  text <- obj.info[[i]][1]
  data <- readRDS(obj.info[[i]][2])
