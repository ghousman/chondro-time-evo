...
#regout <- "reg"
regout <- "non" ###DON'T SCALE DATA FOR NOW (BUT DO THAT LATER)
...
sbatch --account=pi-gilad ./DEdream.sh total.pseudo.logcpm0.fml7.nofc.bh total pseudo TRUE logcpm 0 formula7 stage
...
compare <- readRDS("./data/de-data/06_DEdream.total.pseudo.logcpm0.fml7.nofc.bh.stage.rds")

#compare <- readRDS("./data/de-data/06_DEdream.chondro.pseudo.logcpm0.fml7.nofc.bh.chondrostage.rds")
...
DEgenes$cell.subset <- factor(DEgenes$cell.subset,
                              levels=c("d00","d0","d7.m","d14.m","d7.c","d14.c"))
...
ggplot(DEgenes, aes(x=cell.subset, fill=cell.subset)) +
  ...
  scale_fill_manual(name="Stage",
                    labels=c("d00","d0","d7.m","d14.m","d7.c","d14.c"),
                    values=c(rgb(186, 85,211, maxColorValue=255),
                             rgb(218, 75,180, maxColorValue=255),
                             rgb(200, 50,120, maxColorValue=255),
                             rgb(182, 25, 60, maxColorValue=255),
...
# Overlap of DE genes
combined <- reduce(list(data.frame(gene=DEgenes %>% filter(cell.subset=="d00") %>% dplyr::select(gene), "d00"=1),
                        data.frame(gene=DEgenes %>% filter(cell.subset=="d0") %>% dplyr::select(gene), "d0"=1),
                        data.frame(gene=DEgenes %>% filter(cell.subset=="d7.m") %>% dplyr::select(gene), "d7.m"=1),
                        data.frame(gene=DEgenes %>% filter(cell.subset=="d14.m") %>% dplyr::select(gene), "d14.m"=1),
                        data.frame(gene=DEgenes %>% filter(cell.subset=="d7.c") %>% dplyr::select(gene), "d7.c"=1),
                        data.frame(gene=DEgenes %>% filter(cell.subset=="d14.c") %>% dplyr::select(gene), "d14.c"=1)
                        ), full_join)
...
upset(combined,
      ...
      intersections=list(list("d00"),
                         ...
                         list("d7.m","d14.m"),
                         list("d7.c","d14.c"),
                         list("d00","d0","d7.m","d14.m","d7.c","d14.c")),
      text.scale=2)
